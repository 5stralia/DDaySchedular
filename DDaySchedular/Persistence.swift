...
struct PersistenceController {
    ...
    static var preview: PersistenceController = {
        ...
        for index in 0..<10 {
            ...
            newItem.timestamp = Calendar.current.date(byAdding: DateComponents(day: -index), to: Date())
            newItem.title = "\(index)번, TEST"
            newItem.reminders = [-100, 100]
        }
        do {
    ...
    init(inMemory: Bool = false) {
        container = NSPersistentCloudKitContainer(name: "DDaySchedular")
        if inMemory {
